...
void set_av_codec_ctx(AVCodecContext *c, const std::string &name, int kbs,
  ...
  if (name.find("h264") != std::string::npos) {
    ...
    // FF_PROFILE_HEVC_MAIN_10    4096x2304 8bit & 10bit
    // FF_PROFILE_HEVC_MAIN       4096x2304 8bit
    c->profile = FF_PROFILE_HEVC_MAIN_10;
  }
}

bool set_level(void *priv_data, const std::string &name) {
  int ret;
  if (name.find("nvenc") != std::string::npos) {
    if ((ret = av_opt_set(priv_data, "level", "5.1", 0)) < 0) {
      LOG_ERROR("nvenc set opt level faieled, ret = " +
                av_err2str(ret));
      return false;
    }
  }
  ...
  if (name.find("nvenc") != std::string::npos) {
    ...
    case Quality_High:
      if ((ret = av_opt_set(priv_data, "preset", "p5", 0)) < 0) {
        LOG_ERROR("nvenc set opt preset p5 failed, ret = " + av_err2str(ret));
        return false;
      }
      break;
    case Quality_Medium:
